...
import sys
import math
from CompatibleRelationsGenerator import DataLoader
...
class TripleManager():
    ...
    def get_corrupted2(self, h, r, t, type='head', corruption_mode='sensical'):
        """
        Generate corrupted entities for negative sampling.

        :param h: Head entity of the triple.
        :param r: Relation of the triple.
        :param t: Tail entity of the triple.
        :param type: Whether to corrupt the head or the tail.
        :param corruption_mode: 'sensical' or 'non_sensical'.
        :return: Set of corrupted entities.
        """

        if corruption_mode == "sensical":
            # Use neighborhood-based corruption (NLCWA) to maintain semantics
            if type == "head":
                corrupted = set(self.headEntities[r]) - self.headDict[r][t]
                # Expand corruption using compatible relations
                for r_compatible in self.domDomCompatible.get(r, []):
                    corrupted.update(self.headDict.get(r_compatible, {}).keys())
                for r_compatible in self.domRanCompatible.get(r, []):
                    corrupted.update(self.tailDict.get(r_compatible, {}).keys())

            elif type == "tail":
                corrupted = set(self.tailEntities[r]) - self.tailDict[r][h]
                # Expand corruption using compatible relations
                for r_compatible in self.ranRanCompatible.get(r, []):
                    corrupted.update(self.tailDict.get(r_compatible, {}).keys())
                for r_compatible in self.ranDomCompatible.get(r, []):
                    corrupted.update(self.headDict.get(r_compatible, {}).keys())

        elif corruption_mode == "non_sensical":
            # Use completely random corruptions from all entities
            if type == "head":
                corrupted = self.entitySet - {h}
            elif type == "tail":
                corrupted = self.entitySet - {t}
    ...
    #
    # corrupted_entity = triple_manager.next_corrupted(
    #     e=42,  # Sample entity
    #     corrupted_dict=triple_manager.headCorruptedDict,
    #     all_entities=list(triple_manager.entitySet),
    #     these_entities=triple_manager.headDict
    # )
    # print("Global Corrupted Entity:", corrupted_entity)
    #
    #
    # corrupted_entity = triple_manager.next_corrupted(
    #     e=42,
    #     corrupted_dict=triple_manager.headCorruptedDict,
    #     all_entities=list(triple_manager.entitySet - triple_manager.headDict[10]),  # Avoiding known head entities
    #     these_entities=triple_manager.headDict
    # )
    # print("LCWA Corrupted Entity:", corrupted_entity)
    #
    # # Extract from DataLoader
    # domDom = triple_manager.domDomCompatible
    # domRan = triple_manager.domRanCompatible
    # ranDom = triple_manager.ranDomCompatible
    # ranRan = triple_manager.ranRanCompatible
    #
    # relation = 10  # Example relation in a triple (h, r, t)
    # compatible_entities = set(triple_manager.entitySet)
    #
    # # Use compatible relations to expand the corruption space
    # for r in domDom.get(relation, []):
    #     compatible_entities.update(triple_manager.headDict.get(r, set()))
    # for r in domRan.get(relation, []):
    #     compatible_entities.update(triple_manager.tailDict.get(r, set()))
    # for r in ranRan.get(relation, []):
    #     compatible_entities.update(triple_manager.tailDict.get(r, set()))
    # for r in ranDom.get(relation, []):
    #     compatible_entities.update(triple_manager.headDict.get(r, set()))
    #
    # # Generate corrupted entity from expanded set
    # corrupted_entity = triple_manager.next_corrupted(
    #     e=42,
    #     corrupted_dict=triple_manager.headCorruptedDict,
    #     all_entities=list(compatible_entities),
    #     these_entities=triple_manager.headDict
    # )
    #
    # print("Corrupted Entity:", corrupted_entity)
    #
    #
    # triple_manager = TripleManager(path="data/", splits=['train'])
    #
    # # Get sensical corrupted heads
    # sensical_heads = triple_manager.get_corrupted(h=10, r=5, t=20, type="head", corruption_mode="sensical")
    # print("Sensical Corrupted Heads:", sensical_heads)
    #
    # # Get non-sensical corrupted heads
    # nonsensical_heads = triple_manager.get_corrupted(h=10, r=5, t=20, type="head", corruption_mode="non_sensical")
    # print("Non-Sensical Corrupted Heads:", nonsensical_heads)
    #
    # # TODO make my own small dataset and map out everything on paper to verify
    # # Trying an example too see if it even makes sense
    # h, r, t = 10, 5, 20  # Sample triple for corruption
    #
    # # Test sensical corruption
    # sensical_heads = triple_manager.get_corrupted(h=h, r=r, t=t, type="head", corruption_mode="sensical")
    # sensical_tails = triple_manager.get_corrupted(h=h, r=r, t=t, type="tail", corruption_mode="sensical")
    #
    # # Test non-sensical corruption
    # nonsensical_heads = triple_manager.get_corrupted(h=h, r=r, t=t, type="head", corruption_mode="non_sensical")
    # nonsensical_tails = triple_manager.get_corrupted(h=h, r=r, t=t, type="tail", corruption_mode="non_sensical")
    #
    # # Display results
    # print(f"Original triple: (h={h}, r={r}, t={t})")
    # print("\n--- Sensical Corruptions ---")
    # print(f"Sensical Corrupted Heads ({len(sensical_heads)}):", sensical_heads)
    # print(f"Sensical Corrupted Tails ({len(sensical_tails)}):", sensical_tails)
    #
    # print("\n--- Non-Sensical Corruptions ---")
    # print(f"Non-Sensical Corrupted Heads ({len(nonsensical_heads)}):", nonsensical_heads)
    # print(f"Non-Sensical Corrupted Tails ({len(nonsensical_tails)}):", nonsensical_tails)
